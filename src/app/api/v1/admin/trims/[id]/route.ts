...
import { deleteTrim, updateTrim } from '@/lib/domain/catalog';
...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  const result = await updateTrim(guard.admin, id, parsed.data, guard.ip);
  if (!result.ok) {
...
export async function DELETE(request: NextRequest, { params }: Params) {
  ...
  const { id } = await params;
  const result = await deleteTrim(guard.admin, id, guard.ip);
  if (!result.ok) {
