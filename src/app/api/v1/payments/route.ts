...
import { startHold } from '@/lib/domain/payments';
...
/**
 * `POST /api/v1/payments` — بدء حجز.
 *
 * **`Idempotency-Key` إلزامي** (القسم ٦): الشبكة تُعيد الطلب بلا أن
 * يعرف المستخدم، والمتصفّح يُعيده بضغطة تحديث. وغيابه رفضٌ لا تسامح —
 * والتسامح يعني أن أوّل عميل بشبكة سيّئة يدفع مرّتين.
 */
export async function POST(request: NextRequest) {
  ...
  const seen = await checkIdempotency(key, 'payment.hold', raw);
  ...
  const result = await startHold({ ...parsed.data, buyerId: user.id, idempotencyKey: key });
  ...
  const payload = { paymentId: result.paymentId, status: result.status, actionUrl: result.actionUrl };
  await rememberIdempotency(key, 'payment.hold', raw, payload, 201);
