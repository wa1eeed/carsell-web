...
> = {
  /**
   * ثلاثون يومًا — **حدُّ إنذار لا حدُّ كفاية**، والفرق جوهريّ.
   *
   * مرحلة نقل الملكية **لا تُحدَّد بزمن**: تتقدّم بتأكيد إجراءٍ يمرّرها
   * إلى التالية (`advanceStage`)، لا بمهلة تنقضي. فلا مدّةَ حجزٍ
   * **تكفي بالبرهان** — أيّ رقم قد يُتجاوَز، ومسألة «متى» لا «هل».
   *
   * ولهذا لا يُقرأ الرقم «هذا ما يحتاجه الغرض»: ذاك غير معلوم أصلًا.
   * بل «دون هذا الحدّ يصير انقضاء الحجز مُرجَّحًا بما يكفي للتحذير» —
   * فبوابة بطاقة (٧ أو ٦ أيام) لا تسند مرحلةً غير محدودة، ومن يوجّه
   * الضمان إليها يبني وعدًا لا يملك الوفاء به.
   *
   * وتنبيهُ الاقتراب من الانقضاء (`expiringHolds`) **بنيويّ لا احتياطيّ**
   * لهذا السبب: ما دامت المرحلة بلا سقف، فالانقضاء حالٌ متوقَّعة تُدار
   * لا حادثٌ نادر يُستثنى.
   *
   * // DESIGN-Q: الرقم ٣٠ اجتهادٌ لا اشتقاق — والاشتقاق مستحيل ما دامت
   * // المرحلة بلا سقف. يُضبط من توزيع مدد النقل الفعلية حين تتوفّر.
   */
  VEHICLE_ESCROW: {
...
export function eligibility(
  ...
  if (requirement.minHoldDays > 0 && capabilities.maxHoldDays < requirement.minHoldDays) {
    return {
      ...
      warning: `أقصى مدّة حجز ${String(capabilities.maxHoldDays)} يومًا. ومرحلة نقل الملكية تتقدّم بتأكيد إجراء لا بمهلة، فقد تتجاوزها — وحينها يتحوّل الحجز إلى تحصيل مبكّر، وهذا يغيّر معنى الضمان للمشتري.`,
    };
  }
