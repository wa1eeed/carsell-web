...
export function eligibility(
  ...
  if (requirement.minHoldDays > 0 && capabilities.maxHoldDays < requirement.minHoldDays) {
    return {
      eligible: true,
      warning: `أقصى مدّة حجز ${String(capabilities.maxHoldDays)} يومًا، والغرض قد يحتاج ${String(requirement.minHoldDays)} — سيتحوّل الحجز إلى تحصيل مبكّر، وهذا يغيّر معنى الضمان للمشتري.`,
    };
  }
