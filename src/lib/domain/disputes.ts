...
/**
 * ═══ المراحل التي يجوز فيها النزاع ═══
 *
 * **المال في الضمان، وقبل تأكيد نقل الملكية.** وكانت `DONE` منها.
 *
 * وإخراجُها أثرٌ لازم لقرار المصمّم: **الإفراج يتبع تأكيد النقل**،
 * فبعده لا مال محجوز يُجمَّد — و«استرجاعٌ كامل» يصير وعدًا بلا مصدر.
 * ونزاعٌ يُقبَل ولا يُنفَّذ أسوأ من نزاعٍ يُردّ: الأوّل يُنتظر شهرًا
 * ثم يُكتشف أنه كان فارغًا.
 *
 * فالفحص قبل التأكيد لا بعده — والمشتري يعاين المركبة ثم يؤكّد البائع.
 */
const DISPUTABLE_STAGES = ['PAYMENT', 'TRANSFER'] as const;
