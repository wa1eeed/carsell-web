...
## ⛔ حاجز إطلاق: بناء الإنتاج يسقط — **والسبب عِلّة في Next 15.5**

`npm run build` يسقط عند تصدير `/404` و`/500`:
...
**`npm run verify` لا يبني.** الأنواع واللينت والبوابات والاختبارات كلّها
تمرّ. وخطأٌ لا يظهر إلا في بناء الإنتاج يبقى غير مرئيّ حتى لحظة النشر.

### التشخيص — ما استُبعد بالقياس

| الفرضية | الاختبار | النتيجة |
|---|---|---|
| `next-intl/plugin` | بناء بلا `withNextIntl` | يسقط كما هو |
| إصدار React | تثبيت `19.1.1` مؤقّتًا | يسقط كما هو |
| `output: 'standalone'` | تعطيله وحده | يسقط كما هو |
| ملفاتٌ أضفناها | حذفها ثم إعادتها | لا أثر |
| **Next 15.5.22 نفسها** | **بناء على Next 16.2.12** | **الخطأ يختفي تمامًا** |

و`.next-check/server/pages/` يحوي `_app.js` و`_document.js` و`_error.js`
— يولّدها Next لكل مشروع، والعطل في تصديرها لا في شجرتنا. ولا سطر من
كودنا في المقطع الذي يرمي.

**و١٥٫٥٫٢٢ هي آخر ١٥.x** — فلا إصلاح داخل النسخة الكبرى نفسها.

### التذبذب بين `/404` و`/500` ليس إشارة

الصفحتان تُولَّدان بالتوازي، فتُبلَّغ أيّهما سبقت. وكلتاهما تسقط دائمًا،
والاختلاف بين التشغيلات ضجيجٌ لا معلومة. **والقياس بعد `rm -rf .next-check`
وحده** — ذاكرته تعطي نتائج متناقضة تُضلّل التشخيص ساعةً كاملة.

### التوصية: الترقية إلى Next 16

وهي **قرار يستحقّ إذنًا** لا لأنه تقنيّ — بل لأن ترقية إطارٍ كبرى تمسّ
كل شاشة، ولا تُدفع في آخر جلسة بلا تشغيل انحدارٍ كامل ومراجعة بصرية.

وما رُصد أثناء التجربة، ليُبنى عليه:

1. `next.config.ts`: مفتاحا `eslint` و`typescript` لم يعودا في `NextConfig`
   — تنتقل بوابتاهما إلى مكان آخر.
2. `tsconfig.json`: Next 16 يفرض `jsx: react-jsx`.
3. `/_global-error`: صفحةٌ جديدة يولّدها Next 16 ويصدّرها — سقطت في
   التجربة بـ`useContext` على `null`، وكانت `node_modules` حينها مخلوطة
   من ثلاث تثبيتات `--no-save`. **تُعاد التجربة على تثبيت نظيف** قبل
   الحكم عليها.

### حتى تُرقّى

`npm run build` لا يمرّ، و**٢٩ (الأداء) و٣٠ (المراجعة البصرية) موقوفتان
عليه** — كلتاهما تُقاس على بناء إنتاج. والتطوير والاختبارات تعمل كاملة.
