...
### ما استُبعد بالقياس — كلٌّ ببناءٍ نظيف

**البيئة سليمة**: `create-next-app@15.5.22` على هذا الجهاز نفسه، Node ٢٢،
**يبني بنجاح**. ثم أُضيف إليه واحدًا واحدًا فبقي يبني:

| أُضيف إلى المشروع الفارغ | النتيجة |
|---|---|
| React ١٩٫٢٫٨ (بدل ١٩٫١٫٠) | يبني |
| `next-intl` + `[locale]` + مزوّد + وسيط | يبني |
| تخطيط جذرٍ يمرّر `children` | يبني |
| تخطيط ثانٍ يرسم `<html>` خارج `[locale]` (كـ`admin`) | يبني |
| `next/font/google` (Tajawal + Figtree) | يبني |
| `output: 'standalone'` | يبني |
| نقل كل شيء إلى `src/` | يبني |

**ثم حُذف من مشروعنا واحدًا واحدًا فبقي يسقط**:

| حُذف | النتيجة |
|---|---|
| شجرة `admin` كاملة | يسقط |
| شجرة `(site)` كاملة (بقي تخطيط + صفحة تافهة) | يسقط |
| شجرة `api` كاملة | يسقط |
| `not-found` · `global-error` · `robots` · `sitemap` | يسقط |
| تخطيط `[locale]` مستبدلًا بأبسط صورة | يسقط |
| `middleware` مستبدلًا بأبسط صورة | يسقط |
| `next.config.ts` مستبدلًا بأبسط صورة | يسقط |
| `prisma.config.ts` | يسقط |
| `postcss.config.mjs` (Tailwind v4) | يسقط |
| كل التبعيات عدا الأربع الأساسية | يسقط |
| `distDir` المخصَّص (`build:check`) — بُني بالافتراضيّ | يسقط |
| `allowJs: false` ⇐ `true` في `tsconfig` | يسقط |
| Next 16.2.12 (ترقية كاملة نظيفة) | يسقط بموضعٍ آخر |
| React ١٩٫١٫١ | يسقط |
| نسخة React مكرّرة (`npm ls`) | نسخة واحدة مُوحَّدة |

**فالمشروع يسقط وهو شبه فارغ، والفارغ الحقيقيّ يبني — والفرق بينهما لم
يُعثَر عليه بعد.**

### الباقي للفحص

ما لم يُقارَن بعد بين الشجرتين: `tsconfig.json` بقيّةَ خياراته (قُورن
`allowJs` و`target` وحدهما) · `next-env.d.ts` · `src/messages/*.json`
(ملفّاتنا كبيرة وتُستورَد ديناميكيًّا في `request.ts`) · `.env` المحمَّل
أثناء البناء.

**والطريق الأقصر**: نسخةٌ من المشروع الفارغ الذي يبني، تُنقل إليها ملفات
مشروعنا ملفًّا ملفًّا حتى تسقط — فالسقوط عند ملفٍ بعينه يسمّي السبب.
