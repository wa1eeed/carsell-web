...
## المشي في رحلات البيع الثلاث — بالنقر لا بالاختبار

DIRECT وNEGOTIATION وAUCTION مُشيت من أوّلها إلى إصدار العقد وكشف
التسوية وقيود الدفتر. والاختبارات كانت تمرّ فوق كل ما وُجد هنا.

**والدفع لا يقع بلا وضعٍ تجريبيّ.** البذرة توجّه `VEHICLE_ESCROW` إلى
`bank_escrow` ولا مُهايئ له، فيردّ `GATEWAY_NOT_CONFIGURED` — وتقف كل
رحلة عند الدفع. `npm run trial:on` يوجّه الأغراض الستّة إلى البوابة
التجريبية، وبها تكتمل الرحلة كاملة. **وهذا هو ما يجب أن تكون عليه
staging** حتى تصل مفاتيح المزوّد.

### ما وُجد

**١ · قرار البائع بعد إغلاقٍ باحتياطي غير مبلوغ — بلا باب.**
`resolveSellerDecision` مبنيّة منذ المهمة ١٩ وتُنشئ الطلب وتخصم عربون
الفائز، **ولا ينادي فرعَ القبول شيءٌ في المنتج**: الوحيد الذي يناديها
`expireSellerDecisions` وتمرّر `false` دائمًا. فكل مزادٍ لم يبلغ
احتياطيَه ينتهي **بالردّ حتمًا** مهما أراد البائع، والمهلة تُعرض في
لوحة الأدمن وحدها — ساعاتٌ تُعدّ لقرارٍ لا يستطيع اتّخاذه.

بُني: `POST /api/v1/auctions/{ref}/decision` و`sellerDecisionView`
و`SellerDecisionPanel` في عمود شاشة المزاد.

**٢ · «نافذة الاسترجاع» وعدٌ أُلغي ولم يُحذف نصّه.** ثلاثة مواضع تقول
للمشتري إن له مهلةً بعد تأكيد الاستلام، والإفراج صار يتبع التأكيد
مباشرةً (مهمّة ٣٧). فالتحذير كان يقول «افتح نزاعًا قبل انقضاء النافذة»
وصوابُه **«قبل أن تؤكّد»** — وبعدها يكون المال قد وصل البائع.

**٣ · شارة اتّجاه العرض تُشتقّ من الدور لا من المُرسِل.** الدور ثابتٌ
طول المفاوضة، فالبائع يقرأ «وارد» على مقابلِه هو والمشتري «مُرسَل» على
ما وصله — والتبويب يفرزهما بـ`sentByMe` أصلًا، فكانت الشارة تناقض
العمود الذي هي فيه. (وهي القاعدة نفسها التي أُصلحت في زرّ القبول.)

**٤ · «في هذه المرحلة منذ لا ساعات».** `Quantity` تصوغ الصفر بصيغة
النفي، فأوّل ساعةٍ من كل طلب — وهي اللحظة التي يُفتح فيها أوّل مرّة —
تقرأ جملةً لا معنى لها. أُضيف `lessThanAnHour`.

**٥ · `trial:reset` يسقط بقيدٍ مرجعيّ.** `removeOrder` كانت تحذف كل
أثرٍ إلّا دفعات الطلب: `main` تحذف دفعات البوابة التجريبية قبلها، فتبقى
كل محاولةٍ سبقت تفعيل الوضع (دفعةٌ `FAILED` عبر `bank_escrow`) مشيرةً
إلى الطلب — فيموت التنظيف في منتصفه ويترك نصف ما صُنع.

### وعربون المزاد وسمٌ لا خصم

`APPLIED` لا يُنقص المبلغ المطلوب، **وهذا هو الصواب اليوم**:
`holdDeposit` تكتب `HELD` بيدها ولا تمرّ ببوابة، فلا مال محجوز يُخصم
منه — ولو خُصم لدفع المشتري أقلّ ولم يصل البائع فرقٌ من أحد. والشاشة
لا تذكر العربون، فلا وعدَ مُخلَف. ويوم يمرّ العربون بالبوابة فعلًا
(`AUCTION_DEPOSIT` موجَّه ومضبوط ولا يناديه أحد) **يُخصم في الموضعين
معًا**، ويُذكر السطر في الشاشة.

## قياس: خمس عشرة قدرةً مبنيّة بلا باب

الصنف تكرّر عشر مرّات، فقِيس آليًّا: كل دالّة نطاق **مصدَّرة تكتب**، ثم
الإغلاق العابر لما يبلغه التطبيق. الحصيلة **١٥ دالّة** لا يبلغها شيء
خارج الاختبارات — أي قدرة مبنيّة ومختبَرة لا زرّ لها:

`saveSegment` · `sendCampaign` · `requestEnvSwitch` · `checkConnection` ·
`grantMarginScheme` · `forfeitDeposit` · `recordSellerPayout` ·
`recordRefund` · `holdEscrow` · `setRouteEnabled` · `setPreference` ·
`updateChannel` · `reconcileGateway` · `cancelInvoice` ·
`extendTransferDeadline`

وتجاوزُها عشرة يعني **النقل أوّلًا ثم البوابة** — تُبنى الأبواب، ثم
يُفعَّل الفحص فلا يعود الصنف.

### والسقوط المتقطّع في `admin-account` — سُمّي وأُغلق

الاختبار يفحص ألّا يكتب الأثرُ الكلمةَ **ولا طولَها**، فيبحث عن `'18'`
في الردّ المُسلسَل كلّه — والبريد يحمل طابعًا زمنيًّا عشوائيًّا
(`pwtest921845619@…`) يقع عليه صدفةً. فيسقط على ما لا يقيسه، وتبدو
الصدفة عطلًا في الكود.

النيّة صحيحة والوسيلة خاطئة: **الطول يُفحص على القيم** بعد استبعاد
البريد، **والكلمة تُفحص نصًّا** (عشرون حرفًا لا تقع صدفة).

وهو نصف السقوط المتقطّع المرصود أعلاه — والنصف الثاني كان عدّادًا عامًّا
في المهل، وقد أُصلح.

## إنذار تسريبٍ كاذب — وكيف كاد يُصدَّق

قِستُ حمولة صفحة السيارة العامّة فوجدت فيها **آيبان البائع وهاتفه
وبريده ورقم الهيكل واللوحة** — صفَّ Prisma كاملًا. وكاد يُبلَّغ عنه
عطلًا أمنيًّا حرجًا.

**والحقيقة: الإنتاج سليم.** ما رأيتُه حمولة تتبُّع مكوّنات الخادم
(`env: "Server"` · `owner` · `stack` · أزمنة) يُصدرها Next في التطوير
وحده لأدوات المطوّر. وبناءُ الإنتاج لا يحمل منها شيئًا — قِيس على
`build:check` مُقدَّمًا من `standalone`.

### والقياس الأوّل كان معطوبًا

```sh
n=$(grep -c -- "$pat" file || echo 0)   # ✗
```

`grep -c` **يخرج بواحد عند الصفر**، فيُضيف `|| echo 0` صفرًا ثانيًا
فيصير `n` سطرين، و`[ "$n" != "0" ]` صادقة دائمًا — **فأعلن التسريب على
كل نمط، بما في ذلك على ملفٍّ نظيف**. والصواب `grep -qF` وفحص الخروج.

**والدرس أعمّ من الصياغة**: فحصٌ يقول «وُجد» في الحالتين لا يفحص شيئًا.
فقبل تصديق نتيجةٍ مقلقة، **مرِّر الفحص على حالةٍ تعرف أنها سليمة** —
فإن قال «وُجد» فيها فالعطل في الفحص لا في المقيس.

### والحارس بُني على الشكل لا على الصفحة

`tests/public-shape.test.ts` يفحص `toPublicDetail` مباشرةً: لا مفتاح
محظور في الشجرة، **ولا قيمة** هاتفٍ أو آيبانٍ في المُسلسَل. والقيمة
تُفحص لأن مفتاحًا يُعاد تسميته (`contact`) يتخطّى فحص الأسماء والرقم
يبقى.
