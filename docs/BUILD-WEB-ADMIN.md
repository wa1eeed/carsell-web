...
`prisma/seed.ts` يزرع: **٤٢ ماركة** بالاسمين — **١٠ منها بأشجار كاملة** (طرازات وفئات بالقيَم الموروثة) و**٣٢ بأسمائها فقط**. الكتالوج الكامل يُستورد لاحقًا عبر A12 (استيراد CSV) — لا تُؤلَّف فئات لا نعرفها، فالفئة الخاطئة تنتشر إلى كل إعلان يُنشر منها · ٣ حسابات أدمن بالأدوار · باقة واحدة مجانية بعمولة ٠٪ · ٩ خدمات بأسعارها · ٣٨ قالب إشعار · ٧ مساحات إعلانية · ٢٤ سؤالًا شائعًا بمواضعها · **٦٠ إعلانًا واقعيًا** موزّعًا على الأنواع والمدن والحالات · ٣ مزادات (جارٍ، قادم، منتهٍ) · ١٢ طلبًا في مراحل مختلفة · تقريرَي فحص كاملين.
...
11. Wc صفحة السيارة + FAQ + JSON-LD. القبول: **لا تظهر `reservePrice` ولا `minAcceptPrice` ولا أي قيمة رقمية تساويهما في أي استجابة عامة أو HTML أو JSON-LD** — و`reserveMet: boolean` مسموح صراحةً (قرار ٢٩). البحث النصّي عن الكلمة **ليس** معيارًا: يمرّ لو سُمّي الحقل `rp`.
